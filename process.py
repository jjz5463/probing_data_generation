...
def split_examples(example, index, sentence_per_class):
    ...
    if 0 <= index < sentence_per_class:
        ...
        split_sentences = re.split(r'\s*Less formal:+\s*', generated_sentences, flags=re.IGNORECASE)
        ...
        split_sentences = re.split(r'\s*Less complex:+\s*', generated_sentences, flags=re.IGNORECASE)
        ...
        split_sentences = re.split(r'\s*Without contraction:+\s*', generated_sentences, flags=re.IGNORECASE)
        ...
        split_sentences = re.split(r'\s*Without number substitution:+\s*', generated_sentences, flags=re.IGNORECASE)
